...
/*!
  \brief control start of session acceleration
  \param[in]  p_session   Pointer to the session
  \param[in]  enable  flag to control acceleration
  \return The return value can be any one of the following:  \n
  - PPA_SUCCESS \n
  - PPA_FAILURE
 */
#ifdef NO_DOXY
extern int32_t (*ppa_hook_session_accel_fn)(PPA_SESSION *p_session,uint8_t enable);
#else
extern int32_t ppa_hook_session_accel_fn(PPA_SESSION *p_session,uint8_t enable);
#endif
