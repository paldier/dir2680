	...
	for (index = 0; index < MAX_TUNNEL_ENTRIES; index++) {
		...
		if ( !t_entry ) {
			...
			switch(entry->tunnel_type) {
				...
				case TUNNEL_TYPE_IP6OGRE:
				case TUNNEL_TYPE_6EOGRE:
					//compare the src and dst address
	...
	for (index = 0; index < MAX_TUNNEL_ENTRIES; index++) {
		...
		if ( !t_entry ) {
			...
			if ((t_entry->tunnel_info.ipsec_hdr.inbound != NULL)
                && (t_entry->tunnel_info.ipsec_hdr.inbound->aalg != NULL)
                && (t_entry->tunnel_info.ipsec_hdr.inbound->ealg != NULL)
			...
			if (( t_entry->tunnel_info.ipsec_hdr.outbound != NULL)
                && (t_entry->tunnel_info.ipsec_hdr.outbound->aalg != NULL)
                && (t_entry->tunnel_info.ipsec_hdr.outbound->ealg != NULL)
...
#ifdef CONFIG_LTQ_PPA_VAP_QOS_SUPPORT
	...
	for (i = 0; i < num_caps;) {
		...
		switch (p_item->caps_list[i].cap) {
	        ...
	        case MPE_DS_QOS:
				...
				route.tc_remark = ((p_item->extmark & TC_MASK) >> TC_START_BIT_POS) - 1;
				...
				if (p_item->br_tx_if)
					...
					ppa_strncpy(route.txifname,
						    ppa_get_netif_name(p_item->tx_if),
						    PPA_IF_NAME_SIZE);
				route.is_rx_logical = p_item->is_rx_logical;
				route.is_tx_logical = p_item->is_tx_logical;
#endif
#endif
...
int32_t update_subif_port_info(struct mc_group_list_item *p_item,
			       ...
			       uint16_t *subif)
{
	...
	for (i = 0; i < PPA_MAX_MC_IFS_NUM; i ++) {
		if (ppa_netif_lookup(p_item->netif[i]->name,
...
int32_t ppa_hsel_add_wan_mc_group(struct mc_group_list_item *p_item)
{
	...
	if (p_item->num_vap > 1) {
		...
		mc.sessionAction = (uint32_t)ppa_session_mc_construct_tmplbuf(p_item, l_dest_list_vap);
#endif
...
void ppa_hsel_del_wan_mc_group(struct mc_group_list_item *p_item)
{
	...
	if (p_item->num_vap > 1) {
		...
		//p_item->caps_list[num_caps++].cap = SESS_MC_DS_VAP;
#if defined CONFIG_LTQ_PPA_MPE_HAL || defined CONFIG_LTQ_PPA_MPE_HAL_MODULE
		ppa_session_mc_destroy_tmplbuf((void *)mc.sessionAction);
#endif
	}
#endif
	if ((p_item->flags & SESSION_ADDED_IN_HW)) {
...
int32_t ppa_set_qos_meter(PPA_QOS_METER_CFG *meter_cfg)
{
	...
	ret = ppa_hsel_set_qos_meter_entry(meter_cfg, 0, PAE_HAL);
	return ret;
}
EXPORT_SYMBOL(ppa_set_qos_meter);

int32_t ppa_set_queue_threshold(PPA_QOS_QUEUE_THRESHOLD_CFG *queue_thr_cfg)
{
	int32_t ret = PPA_SUCCESS;

	ret = ppa_hsel_set_qos_queue_threshold(queue_thr_cfg, 0, TMU_HAL);
	return ret;
}
EXPORT_SYMBOL(ppa_set_queue_threshold);
...
int32_t ppa_add_lro_entry(struct session_list_item * p_item, uint32_t f_test)
{
	int32_t ret = PPA_SUCCESS;
