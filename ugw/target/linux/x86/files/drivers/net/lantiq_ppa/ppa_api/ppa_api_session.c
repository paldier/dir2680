...
#if defined(CONFIG_PPA_INTF_MIB_TIMER) && defined(CONFIG_LTQ_PPA_GRX500) && CONFIG_LTQ_PPA_GRX500
  ...
  ppa_session_list_lock();
    ...
    for ( i = 0; i < NUM_ENTITY(g_session_list_hash_table); i++ )
    {
      ...
      ppa_hlist_for_each_entry(p_item, &g_session_list_hash_table[i], hlist)
   	...
   	if (((p_item->flag2 & SESSION_FLAG2_VALID_IPSEC_OUTBOUND) == SESSION_FLAG2_VALID_IPSEC_OUTBOUND ) || ( (p_item->flag2 & SESSION_FLAG2_VALID_IPSEC_TRANS)  == SESSION_FLAG2_VALID_IPSEC_TRANS)){
    	...
    	if (p_item->tunnel_idx == ~0) {
        	if (__ppa_update_ipsec_tunnelindex(ppa_buf,&tunnel_index) == PPA_SUCCESS)
        	{
         	...
         	else
         	{
                ...
                tunnel_index = p_item->tunnel_idx;
...
#if defined(CONFIG_LTQ_PPA_MPE_IP97)
/*For the L2TP + IPSec encrypted packets don't learn till we get the encrypted packets from EIP97 Engine */
    if ((p_item->flag2) & SESSION_FLAG2_VALID_ESP_INC_NTC){
	goto __PPA_FILTERED;
    }
#endif
	...
	if ( ppa_netif_start_iteration(&pos, &ifinfo) == PPA_SUCCESS )
        {
		        ...
		        for (hal_id=1; hal_id< MAX_HAL;hal_id++)
			{
			      ...
			      if(ppa_drv_hal_hook[hal_id] != NULL) {
 					...
 					ppa_drv_get_generic_itf_mib(&itf_mib, 0,hal_id);
...
#if defined(CONFIG_LTQ_PPA_TCP_LITEPATH) && CONFIG_LTQ_PPA_TCP_LITEPATH 
	if (!(flg2 & SESSION_FLAG2_CPU_BOUND))
		ppa_netif_put(rx_ifinfo);
#endif
    goto __ADD_SESSION_DONE;
  }
...
int32_t ppa_get_underlying_vlan_interface_if_eogre(PPA_NETIF *netif, PPA_NETIF **base_netif, int32_t *isEoGre)
{
	...
	*base_netif = NULL;
	...
	if (netif != NULL) {
		if (ppa_is_gre_netif_type(netif, &isIPv4Gre, (uint8_t *)isEoGre)) {
			*base_netif = netif;
			return PPA_SUCCESS;
		}
	}
...
/* Updated the 6rd destination address */
  ...
  /*
   * Checking for the WAN session and extmark bit 17 to set DS_QOS flag
   */
#if IS_ENABLED(CONFIG_LTQ_PPA_QOS_MPE_ACCEL)
  if (p_item->flags & SESSION_WAN_ENTRY)
  {
    /* DS_QOS valid only for WAN sessions */
    if(p_item->extmark & SESSION_DS_QOS_SET) {
...
int32_t ppa_mc_group_setup(PPA_MC_GROUP *p_mc_group, struct mc_group_list_item *p_item, uint32_t flags)
{
	...
	if( (rx_netif = (PPA_NETIF *)ppa_dev_get_by_name(p_mc_group->src_ifname)) != NULL)
	{
...
static int32_t ppa_capwap_check_dst_itf(PPA_CMD_CAPWAP_INFO *p_capwap_group,struct capwap_group_list_item *p_item)
{
    ...
    for(i = 0; i < p_capwap_group->num_ifs; i ++){
        ...
        {
            if(ppa_mc_itf_get(p_capwap_group->lan_ifname[i], &p_netif_info) != PPA_SUCCESS)
               return PPA_FAILURE;
            if(!(p_netif_info->flags & NETIF_PHYS_PORT_GOT)){
               ppa_netif_put(p_netif_info);
               return PPA_FAILURE;
