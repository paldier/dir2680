...
static int tmu_hal_set_qos_queue_threshold(struct net_device *netdev,
				PPA_QOS_QUEUE_THRESHOLD_CFG *queue_thr_cfg);
...
static int32_t tmu_hal_generic_hook(PPA_GENERIC_HOOK_CMD cmd, void *buffer, uint32_t flag)
{
	...
	if ((res = ppa_drv_register_cap(QUEUE_THRESHOLD, 1, TMU_HAL)) != PPA_SUCCESS) {
		pr_err("ppa_drv_register_cap returned failure for capability QUEUE_THRESHOLD!!!\n");
	}
            return PPA_SUCCESS;
        }
	...
	case PPA_GENERIC_HAL_GET_QOS_MIB:  //get maximum QOS queue number
		...
		TMU_HAL_DEBUG_MSG(TMU_DEBUG_TRACE,"<%s> inf =%s\n", __FUNCTION__, mib_info->ifname);
			...
			return PPA_FAILURE;
            	...
            	ret = tmu_hal_get_qos_mib1(if_dev, mib_info->portid, mib_info->queueid, &mib_info->mib, mib_info->flag );
		if(if_dev)
		dev_put(if_dev);
		return ret;
        }
...
//			return PPA_FAILURE;
	   ...
	   ret = tmu_hal_delete_queue( if_dev, dev_name, cfg->q_id, cfg->priority, 0, cfg->intfId, cfg->flags);
	   if(if_dev)
	   dev_put(if_dev);
	   return ret;
	}
  case PPA_GENERIC_HAL_MOD_SUBIF_PORT_CFG:
	{//mod_subif_port
	   ...
	   ret = tmu_hal_modify_subif_to_port(if_dev, if_dev->name, cfg->port_id, cfg->priority_level, cfg->weight, cfg->flags);
	   if(if_dev)
	   dev_put(if_dev);
	   return ret;
	}
	...
	case PPA_GENERIC_HAL_SET_QOS_QUEUE_THRESHOLD_CFG:
	{
		struct net_device *if_dev = NULL;
		int32_t ret = PPA_SUCCESS;
		PPA_QOS_QUEUE_THRESHOLD_CFG *cfg;
		TMU_HAL_DEBUG_MSG(TMU_DEBUG_TRACE, "PPA_GENERIC_HAL_SET_QOS_QUEUE_THRESHOLD_CFG\n");

		cfg = (PPA_QOS_QUEUE_THRESHOLD_CFG *)buffer;
		...
		ret = tmu_hal_set_qos_queue_threshold(if_dev, cfg);
		if (if_dev)
			dev_put(if_dev);
		return ret;
	}
...
//			TMU_HAL_DEBUG_MSG(TMU_DEBUG_ERR,"%s:%d ERROR Failed to Get TMU Resources\n", __FUNCTION__, __LINE__);
	...
	//		__FUNCTION__, tmu_res->tmu_port,tmu_res->sch_id[0],tmu_res->q_id);
...
//	TMU_HAL_DEBUG_MSG( TMU_DEBUG_TRACE,"<%s> Enq/Deq Counter Mode is =%s \n",__FUNCTION__, (mode==1)?"Bytes":"Packet");
	...
	enq_temp= get_enq_counter(tmu_res->q_id);
	deq_temp= get_deq_counter(tmu_res->q_id);
	if(mode == 0){
		...
		uint32_t flag)
{
	...
	reset_enq_counter(tmu_res->q_id);
	reset_deq_counter(tmu_res->q_id);
...
//			__FUNCTION__,dp_subif.port_id,dp_subif.subif,tmu_res->tmu_port,tmu_res->sch_id[0],tmu_res->q_id);
	dp_subif.subif  = dp_subif.subif >> 8; 
	subif_index = tmu_hal_user_sub_interface_info + (tmu_res->tmu_port * TMU_HAL_MAX_SUB_IFID_PER_PORT) + dp_subif.subif;
		...
		uint32_t flag)
{
	...
	TMU_HAL_DEBUG_MSG( TMU_DEBUG_TRACE,"<%s>For Port: %d subif: %d --> TMU Port: %d SB: %d Q: %d SLvl: %d \n",
			__FUNCTION__,dp_subif.port_id,dp_subif.subif,tmu_res->tmu_port,tmu_res->sch_id[0],tmu_res->q_id,tmu_res->sch_lvl);
	dp_subif.subif  = dp_subif.subif >> 8; 
	subif_index = tmu_hal_user_sub_interface_info + (tmu_res->tmu_port * TMU_HAL_MAX_SUB_IFID_PER_PORT) + dp_subif.subif;
	TMU_HAL_DEBUG_MSG( TMU_DEBUG_TRACE," Sub interface index %p\n",subif_index);
...
tmu_hal_update_base_wfq_sched_weight(uint16_t base_sched)
{
	...
	for (i = 0; i < 8 ; i++) {
		...
		if (ilink.sie != 0) { // input is enabled
			if (ilink.sit == 0) { // input is QID
				total_weight += (1000 /ilink.iwgt);
				leaf_weights[leaf_index++] = (1000 /ilink.iwgt);
	...
	for ( i = 0; i < 8; i++) {
		...
		if (ilink.sie != 0) { // input is enabled
			leaf_weights[leaf_index] = (leaf_weights[leaf_index] * 100) / total_weight;
			...
			ilink.iwgt = (1000 / leaf_weights[leaf_index]);
...
tmu_hal_egress_queue_create(const struct tmu_hal_equeue_create *param)
{
	...
	tmu_hal_queue_track[param->index].prio_weight = (1000 / param->iwgt);
...
/* set_reg_val function sets the register values of thx structure
 * based on the selected register.
 */ 
void set_reg_val(struct tmu_equeue_drop_params *thx, 
			PPA_QOS_QUEUE_THRESHOLD_CFG *queue_thr_cfg, uint32_t index)
{
	switch (index) {
	case 0:
		thx->qtth0 = queue_thr_cfg->qtth0;
		break;
	case 1:
		thx->qtth1 = queue_thr_cfg->qtth1;
		break;
	case 2:
		thx->math0 = queue_thr_cfg->math0;
		break;
	case 3:
		thx->math1 = queue_thr_cfg->math1;
		break;
	default:
		TMU_HAL_DEBUG_MSG(TMU_DEBUG_TRACE, "QoS Queue Threshold in def set_reg_val");
	}
}

/*tmu_hal_set_qos_queue_threshold function is used to set queue threshold of
* a queue, if queue num is zero then it will set threshold of all queues
* present on the interface.
*/
int32_t tmu_hal_set_qos_queue_threshold(struct net_device *netdev,
				PPA_QOS_QUEUE_THRESHOLD_CFG *queue_thr_cfg)
{
	...
	if ((dp_get_netif_subifid(netdev, NULL, vcc,
		0, &dp_subif, flags)) != PPA_SUCCESS) {

		TMU_HAL_DEBUG_MSG(TMU_DEBUG_TRACE, "ERROR Failed to Get Subif Id from dp_get_netif_subifid\n");

		if ((dp_get_port_subitf_via_ifname(queue_thr_cfg->ifname,
			&dp_subif)) != PPA_SUCCESS) {
			TMU_HAL_DEBUG_MSG(TMU_DEBUG_TRACE, "ERROR Failed to Get Subif Id from dp_get_port_subitf_via_ifname\n");
			if ((tmu_hal_get_subitf_via_ifname(queue_thr_cfg->ifname,
				&dp_subif)) != PPA_SUCCESS) {
				TMU_HAL_DEBUG_MSG(TMU_DEBUG_TRACE, "ERROR Failed to Get Subif Id from tmu_hal_get_subitf_via_ifname\n");
				return -TMU_HAL_STATUS_NOT_AVAILABLE;
			}
		}
	}
	dp_subif.subif  = dp_subif.subif >> 8;
	if ((cbm_dp_port_resources_get(&dp_subif.port_id, &nof_of_tmu_ports,
					&tmu_res, 0)) != PPA_SUCCESS) {
		TMU_HAL_DEBUG_MSG(TMU_DEBUG_ERR, "ERROR Failed to Get TMU Resources in meter threshold\n");
		return TMU_HAL_STATUS_ERR;
	}
	...
	if (queue_thr_cfg->queue_num > 0) {
		index = tmu_hal_get_user_index_from_qid(subif_index,
				queue_thr_cfg->qid);
		if (index == TMU_HAL_STATUS_ERR) {
			TMU_HAL_DEBUG_MSG(TMU_DEBUG_TRACE, "Queue Threshold set: Invalid Queue Id\n");
			ret = TMU_HAL_STATUS_INVALID_QID;
		}
		tmu_equeue_drop_params_get(subif_index->user_queue[index].queue_index, &thx);
		if (queue_thr_cfg->col_flag & PPA_QOS_COL0_THRESHOLD) {
			thx.col[0] = queue_thr_cfg->col[0];
			set_reg_val(&thx, queue_thr_cfg, queue_thr_cfg->col[0]);
		}
		if (queue_thr_cfg->col_flag & PPA_QOS_COL1_THRESHOLD) {
			thx.col[1] = queue_thr_cfg->col[1];
			set_reg_val(&thx, queue_thr_cfg, queue_thr_cfg->col[1]);
		}
		if (queue_thr_cfg->col_flag & PPA_QOS_COL2_THRESHOLD) {
			thx.col[2] = queue_thr_cfg->col[2];
			set_reg_val(&thx, queue_thr_cfg, queue_thr_cfg->col[2]);
		}
		if (queue_thr_cfg->col_flag & PPA_QOS_COL3_THRESHOLD) {
			thx.col[3] = queue_thr_cfg->col[3];
			set_reg_val(&thx, queue_thr_cfg, queue_thr_cfg->col[3]);
		}
		TMU_HAL_DEBUG_MSG(TMU_DEBUG_TRACE, "Coloring Queue Threshold of QID %d\n", subif_index->user_queue[index].queue_index);
		tmu_equeue_drop_params_set(subif_index->user_queue[index].queue_index, &thx);
	} else {
		for (i = 0; i < subif_index->queue_cnt; i++) {
			TMU_HAL_DEBUG_MSG(TMU_DEBUG_TRACE, "Coloring Queue Threshold of QID %d\n", subif_index->user_queue[i].queue_index);
			tmu_equeue_drop_params_get(subif_index->user_queue[i].queue_index, &thx);
			if (queue_thr_cfg->col_flag & PPA_QOS_COL0_THRESHOLD) {
				thx.col[0] = queue_thr_cfg->col[0];
				set_reg_val(&thx, queue_thr_cfg, queue_thr_cfg->col[0]);
			}
			...
			tmu_equeue_drop_params_set(subif_index->user_queue[i].queue_index, &thx);
		}
	}
	return ret;
		...
		uint32_t flags)
{
	...
	TMU_HAL_DEBUG_MSG( TMU_DEBUG_TRACE,"<%s> No of entries is %d for queue=%d\n", __FUNCTION__, no_entries, q_in);
		...
		if(ilink.sie != 0) { // input is enabled
			if(ilink.sit == 0) // input is QID
			{
				...
				/*if(cfg_shaper != 0xFF) {
					...
					cfg.flag = flags;
					tmu_hal_add_queue_rate_shaper_ex(netdev, &cfg, q_in, flags);
				}*/
	...
	for(j= i-1; j >= priority; j--) {
		...
		if(ilink.sit == 0) // input is QID
		{
			...
			if(cfg_shaper != 0xFF) {
				...
				cfg.dev_name = dev_name;
				cfg.flag = flags;
				tmu_hal_del_queue_rate_shaper_ex(netdev, &cfg, q_in, flags);
			}
			//tmu_sched_blk_in_enable(sched_input << 3 | j, 0);
			//tmu_hal_egress_queue_delete(q_in);
			//tmu_hal_safe_queue_delete(netdev, q_in, subif_index->default_q);
			tmu_hal_safe_queue_delete(netdev, dev_name, q_in, q_in, flags);
...
//	TMU_HAL_DEBUG_MSG( TMU_DEBUG_TRACE,"<%s> For Datapath Port: %d subif: %d --> TMU Port: %d SB: %d Q: %d SLvl: %d\n",
//			__FUNCTION__,dp_subif.port_id,dp_subif.subif,tmu_res->tmu_port,tmu_res->sch_id[0],tmu_res->q_id,tmu_res->sch_lvl);
...
//			TMU_HAL_DEBUG_MSG( TMU_DEBUG_TRACE,"<%s>For Port: %d subif: %d --> TMU Port: %d SB: %d Q: %d\n",
//					__FUNCTION__, dp_subif.port_id,dp_subif.subif,(tmu_res + i)->tmu_port,(tmu_res+i)->sch_id[0],(tmu_res+i)->q_id);
			if((flags & PPA_QOS_Q_F_INGRESS) == PPA_QOS_Q_F_INGRESS) {
				...
				(tmu_hal_user_sub_interface_ingress_info + ((tmu_res + i)->tmu_port * TMU_HAL_MAX_SUB_IFID_PER_PORT) + dp_subif.subif)->base_sched_id_egress = tmu_res->sch_id[0]; 
				(tmu_hal_user_sub_interface_ingress_info + ((tmu_res + i)->tmu_port * TMU_HAL_MAX_SUB_IFID_PER_PORT) + dp_subif.subif)->user_queue[0].queue_index = tmu_res->q_id; 
				...
				(tmu_hal_user_sub_interface_ingress_info + ((tmu_res + i)->tmu_port * TMU_HAL_MAX_SUB_IFID_PER_PORT) + dp_subif.subif)->user_queue[0].sbin = tmu_res->sch_id[0] << 3; 
				...
				(tmu_hal_user_sub_interface_info + ((tmu_res + i)->tmu_port * TMU_HAL_MAX_SUB_IFID_PER_PORT) )->base_sched_id_egress = (tmu_res+i)->sch_id[0]; 
				(tmu_hal_user_sub_interface_info + ((tmu_res + i)->tmu_port * TMU_HAL_MAX_SUB_IFID_PER_PORT) )->user_queue[0].queue_index = (tmu_res+i)->q_id; 
				...
				(tmu_hal_user_sub_interface_info + ((tmu_res + i)->tmu_port * TMU_HAL_MAX_SUB_IFID_PER_PORT) )->user_queue[0].sbin = (tmu_res+i)->sch_id[0] << 3; 
				...
				(tmu_hal_user_sub_interface_info + ((tmu_res + i)->tmu_port * TMU_HAL_MAX_SUB_IFID_PER_PORT) )->default_q = (tmu_res+i)->q_id; 
			...
			tmu_hal_port_track[(tmu_res+i)->tmu_port].input_sched_id = (tmu_res+i)->sch_id[0];
			tmu_hal_port_track[(tmu_res+i)->tmu_port].default_sched_id = (tmu_res+i)->sch_id[0];
			...
			tmu_hal_sched_track[(tmu_res+i)->sch_id[0]].in_use = 1;
			tmu_hal_sched_track[(tmu_res+i)->sch_id[0]].id = (tmu_res+i)->sch_id[0];
			tmu_hal_sched_track[(tmu_res+i)->sch_id[0]].level = 0;
			tmu_hal_sched_track[(tmu_res+i)->sch_id[0]].policy = TMU_HAL_POLICY_STRICT;
			tmu_hal_sched_track[(tmu_res+i)->sch_id[0]].leaf_mask = 0x01;
			tmu_hal_sched_track[(tmu_res+i)->sch_id[0]].next_sched_id = 0xFF;
			tmu_hal_sched_track[(tmu_res+i)->sch_id[0]].peer_sched_id = 0xFF;
			tmu_hal_sched_track[(tmu_res+i)->sch_id[0]].sched_out_conn = 0; // default connected to the port
			tmu_hal_sched_track[(tmu_res+i)->sch_id[0]].omid = (tmu_res+i)->tmu_port;
			tmu_hal_sched_track[(tmu_res+i)->sch_id[0]].tbs = 0xFF;


			if(flag == DP_F_MPE_ACCEL) {
				tmu_hal_sched_track[(tmu_res+i)->sch_id[0]].policy = TMU_HAL_POLICY_WFQ;
			}

			/** Update the Queue info*/
			tmu_hal_queue_track[(tmu_res+i)->q_id].is_enabled = 1;
			tmu_hal_queue_track[(tmu_res+i)->q_id].connected_to_sched_id = (tmu_res+i)->sch_id[0];
			tmu_hal_queue_track[(tmu_res+i)->q_id].epn = (tmu_res+i)->tmu_port;
			tmu_hal_queue_track[(tmu_res+i)->q_id].q_type = TMU_HAL_POLICY_STRICT;
			tmu_hal_queue_track[(tmu_res+i)->q_id].prio_weight = 0;
...
int32_t tmu_hal_init_ingress_index( struct tmu_hal_user_subif_abstract subif_index)
{
	if(subif_index->base_sched_id_egress == 0xFF) {
		...
		(tmu_hal_user_sub_interface_ingress_info + (tmuport_index * TMU_HAL_MAX_SUB_IFID_PER_PORT) )->base_sched_id_egress = tmu_res->sch_id[0]; 
		(tmu_hal_user_sub_interface_ingress_info + (tmuport_index * TMU_HAL_MAX_SUB_IFID_PER_PORT) )->user_queue[0].queue_index = tmu_res->q_id; 
		...
		(tmu_hal_user_sub_interface_ingress_info + (tmuport_index * TMU_HAL_MAX_SUB_IFID_PER_PORT) )->user_queue[0].sbin = tmu_res->sch_id[0] << 3; 
		...
		uint32_t flags)
{
	...
	if((flags & PPA_QOS_Q_F_INGRESS) == PPA_QOS_Q_F_INGRESS) {
		...
		dp_subif.subif  = dp_subif.subif >> 8;
		TMU_HAL_DEBUG_MSG( TMU_DEBUG_TRACE,"<%s> CPU Ingress --> For Port: %d subif: %d --> TMU Port: %d SB: %d Q: %d SLvl: %d No Of TMU ports: %d\n",
				__FUNCTION__,dp_subif.port_id,dp_subif.subif,tmu_res->tmu_port,tmu_res->sch_id[0],tmu_res->q_id,tmu_res->sch_lvl,nof_of_tmu_ports);
		tmuport = tmu_res->tmu_port;
		kfree(tmu_res);
	}
	...
	if((flags & PPA_QOS_Q_F_INGRESS) == PPA_QOS_Q_F_INGRESS)
	{
		...
		if(!g_IngressQueueScheduler) {
			...
			if(tmu_hal_get_tmu_res_from_netdevice(netdev, netdev->name, schedid, &dp_subif, &nof_of_cpu_tmu_ports, &cpu_tmu_res, (flags & ~DP_F_MPE_ACCEL | PPA_QOS_Q_F_INGRESS)) == TMU_HAL_STATUS_ERR) {
				TMU_HAL_DEBUG_MSG( TMU_DEBUG_TRACE,"%s:%d Error: Failed to get resources from Netdevice\n", __FUNCTION__, __LINE__);
				kfree(cpu_tmu_res);
				return TMU_HAL_STATUS_ERR;
			}
			...
			q_index = cpu_tmu_res->q_id; // this is the QID						
			TMU_HAL_DEBUG_MSG( TMU_DEBUG_TRACE,"Already queue %d is attached to this leaf\n",q_index);
		...
		uint32_t flags)
{
	...
	if((flags & PPA_QOS_Q_F_INGRESS) == PPA_QOS_Q_F_INGRESS)
	{
		...
		if(port_index->base_sched_id_egress == 0xFF) {
			TMU_HAL_DEBUG_MSG( TMU_DEBUG_TRACE,"Create Port Scheduler for the port \n");
			sched_port = create_new_scheduler(cpu_tmu_res->sch_id[0], 0, TMU_HAL_POLICY_WFQ, tmu_hal_sched_track[cpu_tmu_res->sch_id[0]].level + 1, 1);
			tmu_hal_sched_track[sched_port].policy = TMU_HAL_POLICY_STRICT;
	...
	if((dp_subif.port_id !=0) && (tmu_res->tmu_port != g_CPU_PortId) ) { // Egress handling except CPU port
		...
		if(dp_subif.subif != 0) // for the logical interface
		{
			...
			if(port_prio_level > high_prio_q_limit) {
				if(nof_of_tmu_ports > 1) {
					port_sched = base_sched = (tmu_res)->sch_id[0];
					tmuport = (tmu_res)->tmu_port;
					subif_index = tmu_hal_user_sub_interface_info + (tmuport * TMU_HAL_MAX_SUB_IFID_PER_PORT) + dp_subif.subif;
				...
				if(nof_of_tmu_ports > 1) {
					port_sched = base_sched = (tmu_res+1)->sch_id[0];
					tmuport = (tmu_res+1)->tmu_port;
					subif_index = tmu_hal_user_sub_interface_info + (tmuport * TMU_HAL_MAX_SUB_IFID_PER_PORT) + dp_subif.subif;
		...
		uint32_t flags)
{
	...
	//tmu_hal_egress_queue_delete(q_index);
	//tmu_hal_safe_queue_delete(netdev, q_index, -1, q_index);
	tmu_hal_safe_queue_and_shaper_delete(netdev, dev_name, q_index, -1, cfg_shaper,  -1, flags);
...
//	devname = netdev->name;
	...
	tmu_hal_shift_up_sbin_conn(netdev, tmuport, devname, subif_index, subif_index->user_queue[index].sbin, flags);
	subif_index->queue_cnt--;
#endif
	{
		if(prio == subif_index->default_prio ) {
...
int tmu_hal_modify_queue(struct net_device *netdev, QOS_Q_MOD_CFG *param)
{
	...
	if(subif_index->user_queue[index].prio_level != param->priority) {
		...
		if(tmu_hal_delete_queue(netdev,netdev->name, param->q_id,subif_index->user_queue[index].prio_level, 0, 0, param->flags) == TMU_HAL_STATUS_OK)
		{
			...
			//param->q_id = tmu_hal_add_queue(netdev, netdev->name, 0, 1, 1, param->priority, param->weight, param->priority, 0, param->flags);
...
//	dev_name = netdev->name;
	...
	TMU_HAL_DEBUG_MSG( TMU_DEBUG_TRACE,"<%s> %d No of entries is %d for queue=%d\n", __FUNCTION__,__LINE__, num_entries, q_index);
	TMU_HAL_DEBUG_MSG( TMU_DEBUG_TRACE,"<%s> Q map pointer =%p\n",__FUNCTION__, q_map_get);
//	TMU_HAL_DEBUG_MSG( TMU_DEBUG_TRACE,"<%s> Delete Queue %d for netdev %s dev_name = %s\n",__FUNCTION__, q_index, netdev->name, dev_name);
	if(tmu_hal_flush_queue_if_qocc(netdev, dev_name, q_index,-1, -1, flags) != TMU_HAL_STATUS_OK) {
		TMU_HAL_DEBUG_MSG(TMU_DEBUG_ERR,"%s:%d ERROR to flush qocc\n", __FUNCTION__, __LINE__);
		ret = TMU_HAL_STATUS_ERR;
		goto CBM_RESOURCES_CLEANUP;
...
#else					
					if(tmu_hal_queue_enable_disable_if_qocc(netdev, dev_name, subif_index->user_queue[i].queue_index, -1, -1, flags) != TMU_HAL_STATUS_OK)
					{
						TMU_HAL_DEBUG_MSG(TMU_DEBUG_TRACE,"%s:%d ERROR to flush qocc by disabling \n", __FUNCTION__, __LINE__);

						tmu_hal_enable_disable_queue(subif_index->user_queue[i].queue_index, 1);
						if(tmu_hal_flush_queue_if_qocc(netdev, dev_name, subif_index->user_queue[i].queue_index, -1, -1, flags) != TMU_HAL_STATUS_OK) {
							TMU_HAL_DEBUG_MSG(TMU_DEBUG_TRACE,"%s:%d ERROR to flush qocc\n", __FUNCTION__, __LINE__);
							return TMU_HAL_STATUS_ERR;
						}
						//return TMU_HAL_STATUS_ERR;
...
int tmu_hal_get_queue_num(struct net_device *netdev, int32_t portid, int32_t *q_num)
{
	...
	TMU_HAL_DEBUG_MSG( TMU_DEBUG_HIGH,"<%s>For Port: %d subif: %d --> TMU Port: %d SB: %d Q: %d SLvl: %d\n",
				__FUNCTION__,dp_subif.port_id,dp_subif.subif,tmu_res->tmu_port,tmu_res->sch_id[0],tmu_res->q_id,tmu_res->sch_lvl);
	subif_index = tmu_hal_user_sub_interface_info + (tmu_res->tmu_port * TMU_HAL_MAX_SUB_IFID_PER_PORT) + dp_subif.subif;
			...
			uint32_t pmac_port)
{
    ...
    if(g_DpIngressQueueScheduler == 0xFF) {
		...
		TMU_HAL_DEBUG_MSG( TMU_DEBUG_TRACE,"<%s> TMU resources for Directpath Rx is Port:%d ,SB:%d, Q:%d SLvl: %d\n",
				__FUNCTION__,tmu_res->tmu_port,tmu_res->sch_id[0],tmu_res->q_id,tmu_res->sch_lvl);
		g_DpIngressQueueScheduler = tmu_res->sch_id[0]; 
		g_Port_id_DpIngress = tmu_res->tmu_port; 
		g_Queue_id_DpIngress = tmu_res->q_id; 
		q_new = tmu_res->q_id; 
	...
	*/
	...
	printk( "<%s> TMU resources for Checksum is Port:%d ,SB:%d, Q:%d SLvl: %d\n",
			__FUNCTION__,tmu_res->tmu_port,tmu_res->sch_id[0],tmu_res->q_id,tmu_res->sch_lvl);
...
int hal_init(void)
{
...

	{
		...
		g_CPU_Sched = tmu_res->sch_id[0];
		g_CPU_Queue = tmu_res->q_id;
		...
		g_MPE_Sched = tmu_res->sch_id[0];
		g_MPE_Queue = tmu_res->q_id;
