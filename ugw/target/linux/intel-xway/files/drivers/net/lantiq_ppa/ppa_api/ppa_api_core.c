...
/* Search for the p_item only if ptr_ppa_pitem pointer is null in the skb  */
        if (ppa_buf->ptr_ppa_pitem) {
        	p_item = (struct session_list_item *)(ppa_buf->ptr_ppa_pitem);
/* To handle the situation when p_item got deleted but skb still holds the reference to it */
		if(ppa_atomic_read(&p_item->used) <= 0)
		{
			ppa_buf->ptr_ppa_pitem = NULL;
			return PPA_SESSION_NOT_ADDED;
		}
		ppa_session_list_lock();
  		ppa_atomic_inc(&p_item->used); 
		...
		if (PPA_SESSION_EXISTS == ret){
  		/* Store the pointer to p_item in skb */ 
  			ppa_buf->ptr_ppa_pitem = p_item;
		}
	}
...
int32_t mcast_module_config_puma(uint32_t grp_idx, struct net_device *member, mcast_stream_t *mc_stream, uint32_t flags)
{
#ifdef CONFIG_TI_PACKET_PROCESSOR
	struct hil_mr_ipaddr hil_ip;

	memset(&hil_ip, 0, sizeof(hil_ip));

	if (mc_stream->src_ip.ipType == 0) {
		hil_ip.type = HIL_MR_IPV4;
		hil_ip.addr.ip4.s_addr = mc_stream->dst_ip.ipA.ipAddr.s_addr;
	} else {
		hil_ip.type = HIL_MR_IPV6;
		ppa_memcpy(&(hil_ip.addr.ip6), &(mc_stream->dst_ip.ipA.ip6Addr.s6_addr[0]), sizeof(struct in6_addr));
	}
	hil_ip.vpid = member->vpid_handle;

	if (flags & MC_F_DEL) {
		ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT, "hil_ip.type: %d vpid: %d/%s [%s] src: %X dest: %X\n",
			hil_ip.type, hil_ip.vpid, member->name, "DEL", hil_ip.addr.ip4.s_addr, mc_stream->dst_ip.ipA.ipAddr.s_addr);
		ti_hil_pp_event(MC_SESSION_DEL_MEMBER, (void *)&hil_ip);
	} else {
		ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT, "hil_ip.type: %d vpid: %d/%s [%s] src: %X dest: %X\n",
			hil_ip.type, hil_ip.vpid, member->name, "ADD", hil_ip.addr.ip4.s_addr, mc_stream->dst_ip.ipA.ipAddr.s_addr);
...
EXPORT_SYMBOL(mcast_module_config_puma);
...
/*API to control start of acceleration by set/reset flag SESSION_NOT_ACCELABLE 
enable 1 - accelerate the session
	   0 - do not accelerate the session
*/
int32_t ppa_session_accel(PPA_SESSION *p_session, uint8_t enable)
{
    ...
    if ( __ppa_session_find_by_ct(p_session, 0, &p_item) == PPA_SESSION_EXISTS ) {
		/*enable is 1 and SESSION_NOT_ACCELABLE is set, then clear the flag to start
		accelerating the session*/
		if( (enable) && (p_item->flags & SESSION_NOT_ACCELABLE) ){
			p_item->flags &= ~SESSION_NOT_ACCELABLE;	
		}
		else /*enable is 0, set the flag to not accelerate the session*/
			p_item->flags |= SESSION_NOT_ACCELABLE;
	}
	else 
		ret = PPA_FAILURE;
