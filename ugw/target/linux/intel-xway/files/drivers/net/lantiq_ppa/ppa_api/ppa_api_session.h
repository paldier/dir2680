...
struct session_list_item {
	...
	uint16_t                    dest_ifid;
	uint32_t                    flags;
	//  Internal flag : SESSION_IS_REPLY, SESSION_IS_TCP,
...
int32_t ppa_postrouting_speed_handle_frame(PPA_BUF *, struct session_list_item *, uint32_t);
int32_t ppa_prerouting_speed_handle_frame(PPA_BUF *, struct session_list_item *, uint32_t);
...
int32_t ppa_bridging_flush_sessions(void);
