...
#if defined(CONFIG_LTQ_PPA_API_PROC) || defined(CONFIG_LTQ_PPA_API_PROC_MODULE)
...
#endif
int32_t ppa_ioctl_check_kernel_config(unsigned int cmd);
		...
		for(i=0; i<tmp_num; i++) {
			...
			if ( pp_item->flags & session_flag ) {
						  ...
						  !(pp_item->flags & SESSION_ADDED_IN_HW ) && !(pp_item->flags & SESSION_ADDED_IN_SW ) ) )
				{
					...
					ppa_memcpy( &pSessIn->session_list[0].natip, &pp_item->nat_ip, sizeof(pSessIn->session_list[0].natip));
...
int32_t ppa_ioctl_check_kernel_config(unsigned int cmd)
{
	...
	case PPA_CMD_DEL_CLASSIFIER:
		...
		ppa_debug(DBG_ENABLE_MASK_ERR, "This is only for GRX500 platform based boards. Please check whether GRX500 platform is used before firing this ioctl command\n");
		goto EXIT_CHECK;
#endif
		break;
...
#ifndef MBR_CONFIG
		ppa_debug(DBG_ENABLE_MASK_ERR, "Please enable MBR_CONFIG macro before firing this ioctl command\n");
		goto EXIT_CHECK;
#endif
		break;
	case PPA_CMD_SET_MIB_MODE:
	case PPA_CMD_GET_MIB_MODE:
#ifndef MIB_MODE_ENABLE
		ppa_debug(DBG_ENABLE_MASK_ERR, "Please enable MIB_MODE_ENABLE macro before firing this ioctl command\n");
		goto EXIT_CHECK;
#endif
		break;
	case PPA_CMD_SET_RTP:
#ifndef RTP_SAMPLING_ENABLE
		ppa_debug(DBG_ENABLE_MASK_ERR, "Please enable RTP_SAMPLING_ENABLE macro before firing this ioctl command\n");
		goto EXIT_CHECK;
#endif
		break;
...
#ifndef CAP_WAP_CONFIG
		ppa_debug(DBG_ENABLE_MASK_ERR, "Please enable CAP_WAP_CONFIG in kernel config before firing this ioctl command\n");
		goto EXIT_CHECK;
#endif
		break;
	...
	case PPA_CMD_GET_SW_SESSION_ENABLE:
		...
		ppa_debug(DBG_ENABLE_MASK_ERR, "Please enable CONFIG_LTQ_PPA_API_SW_FASTPATH in kernel config before firing this ioctl command\n");
		goto EXIT_CHECK;
#endif
		break;
...
#if !IS_ENABLED(CONFIG_LTQ_PPA_HANDLE_CONNTRACK_SESSIONS)
		ppa_debug(DBG_ENABLE_MASK_ERR, "Please enable CONFIG_LTQ_PPA_HANDLE_CONNTRACK_SESSIONS in kernel config before firing this ioctl command\n");
		goto EXIT_CHECK;
#endif
		break;
	default:
		ppa_debug(DBG_ENABLE_MASK_DEBUG2_PRINT, "Kernel Macros checking done for ioctl commands\n");
	}
	return ret;

EXIT_CHECK:
	ret = PPA_UNSUPPORTED_IOCTL;
	return ret;
}
...
#endif
{
    ...
    if (ppa_ioctl_check_kernel_config(cmd) == PPA_UNSUPPORTED_IOCTL)
    {
        goto EXIT_EINVAL;
    }
    ...
    switch ( cmd )
	{
		...
		case PPA_CMD_SET_QOS_QUEUE_THR:
			{
				res = ppa_ioctl_set_queue_threshold(cmd, arg, cmd_info);
				break;
			}
#endif
		default:
...
{
	ppa_reg_export_fn(PPA_SESSION_ACCEL_FN,   (uint32_t)ppa_session_accel, "ppa_session_accel",(uint32_t *)&ppa_hook_session_accel_fn, (uint32_t)ppa_session_accel_rpfn );
	...
	if (ppa_session_bridged_flow_status())
	...
	//extern uint32_t ppa_directpath_register_dev_rpfn, ppa_directpath_send_rpfn, ppa_directpath_rx_stop_rpfn,ppa_directpath_rx_restart_rpfn,ppa_get_ifid_for_netif_rpfn;
