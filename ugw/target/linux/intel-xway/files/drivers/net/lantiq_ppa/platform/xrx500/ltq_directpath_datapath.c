...
/*
Define a macro ENABLE_DEBUG_INFO to enable debug prints
                        ...
                        uint32_t pmac_port)
{
	...
	if(pmac_port < MAX_DIRECTPATH_NUM)
		g_q_conn_status[pmac_port].egress_q =1;
	else
		pr_err(" pmac port is not less than %d\n", MAX_DIRECTPATH_NUM);
	return 0;
}
	...
	else // for unregister
	{
		...
		if(p_subif->subif == -1){ //Only required when physical port is removed
			...
			*index = port_index;
			...
			if(module_index < MAX_DIRECTPATH_NUM )
				memset(&g_ltq_dp_module[module_index], 0 , sizeof(struct module));
			else
				pr_err(" module_index is not less than %d\n", MAX_DIRECTPATH_NUM);
		} else {
			struct ppe_directpath_data *priv_subif = NULL;
			subif_index = port_index + 1 + p_subif->subif;
...
int32_t ltq_directpath_send(PPA_SUBIF *p_subif, PPA_SKBUF *skb, int32_t len, uint32_t flags)
{
	int32_t port_index, subif_index;
	struct ppe_directpath_data *priv;
