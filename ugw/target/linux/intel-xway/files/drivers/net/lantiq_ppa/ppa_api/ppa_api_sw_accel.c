...
#if IS_ENABLED(CONFIG_LTQ_PPA_SGA)
static bool g_sga_hook_enabled __read_mostly;
#endif

#if defined(CONFIG_LTQ_PPA_TCP_LITEPATH) && CONFIG_LTQ_PPA_TCP_LITEPATH
volatile unsigned char g_tcp_litepath_enabled=0;
#endif
...
#define IsReplySession(flags)          ( (flags) & SESSION_IS_REPLY)
...
#ifdef CONFIG_PPA_PUMA7	
			PPA_NETIF *netif = skb->dev;
			skb->dev = ppa_get_netif(tx_ifinfo->name);
			if(IsLanSession(p_item->flags) && IsPppoeSession(p_item->flags)) {
				if (ppa_pppoe_get_physical_if(skb->dev, NULL, underlying_ifname) == PPA_SUCCESS) {
					skb->dev = ppa_get_netif(underlying_ifname);
				}
			}
			if (NULL != skb->dev->qos_select_hook) {
				swaHdr.queue = skb->dev->qos_select_hook(skb);
			}
			skb->dev = netif;
#endif
    }
...
//swa_dump_skb(128,skb);
	...
	if( !(p_item->flags & SESSION_TX_ITF_IPOA_PPPOA_MASK) )  /* put ethernet header */
	{
		...
		if ((tx_ifinfo && tx_ifinfo->flags & NETIF_L2NAT) && (swaHdr.tx_if != NULL)) {
...
/* Skip CPU bound Nated trafic  */
	if (p_item->flags & SESSION_VALID_NAT_IP || p_item->flags & SESSION_VALID_NAT_PORT)
	    return PPA_FAILURE; 
...
#if IS_ENABLED(CONFIG_LTQ_PPA_SGA)
static int32_t enable_sga_hook(uint32_t f_enable, uint32_t flags)
{
	g_sga_hook_enabled = !!f_enable;
	return 0;
}

static int32_t get_sga_hook(uint32_t *f_enable, uint32_t flags)
{
	if (f_enable) {
		*f_enable = g_sga_hook_enabled;
	}
	return 0;
}
#endif
...
#ifdef CONFIG_PPA_PUMA7	
	if (swa->queue)
		SKB_GET_PP_INFO_P(skb)->egress_queue = swa->queue;
#endif
...
#if IS_ENABLED(CONFIG_LTQ_PPA_SGA)
	if (g_sga_hook_enabled && p_item->session) {
		sga_update_byte_count(p_item->session, 1 /* 1 packet */, skb->len, (IsReplySession(p_item->flags) ? IP_CT_DIR_REPLY : IP_CT_DIR_ORIGINAL));
	}
#endif
...
//skip the telnet sessions ; tcp port 23
	if(p_item->src_port==23 || p_item->dst_port==23) {
            p_item->flags |= SESSION_NOT_ACCELABLE; //to avoid hitting the complete learning path again
	    printk(KERN_INFO"!!!!!!!!cannot accelerate telnet session %p\n!!!!!!!", p_item->session);
	    return PPA_FAILURE;
	}
  }
#endif
...
#if IS_ENABLED(CONFIG_LTQ_PPA_SGA)
extern int32_t (*ppa_sga_enable_hook)(uint32_t, uint32_t);
extern int32_t (*ppa_sga_status_hook)(uint32_t *, uint32_t);
#endif
...
#if IS_ENABLED(CONFIG_LTQ_PPA_SGA)
  ppa_sga_status_hook = get_sga_hook;
  ppa_sga_enable_hook = enable_sga_hook;
#endif
...
{
#if IS_ENABLED(CONFIG_LTQ_PPA_SGA)
  ppa_sga_enable_hook = NULL;
  ppa_sga_status_hook = NULL;
#endif
