...
Data.prototype = {
	...
	showExtenderInfo: function(displayCard) {
		...
		if(objSignal > 67) {
			signalQulity = "Excellent";
		} else if(objSignal <= 67 && objSignal > 33) {
			signalQulity = "Good";
		} else if(objSignal <= 33 && objSignal > 0) {
			signalQulity = "Weak";
		} else {
			signalQulity = "Disconnect";
			textColor = "color:#f00";
			cssBackGround += "_Offline";
		}
		...
		var iconBgPos = "0px";
		if(extenderIconImg[clearcode(self.DeviceName)] != undefined) {
			iconBgPos = extenderIconImg[clearcode(self.DeviceName)];
		}
		...
		+ '<div class="' + cssBackGround + '" style="background-position-y:' + iconBgPos + '"></div>'
		...
		+ '<div class="client_ExtenderQulity">'+ I18N("j", "Quality") + ': '
		+ '<div class="client_signalQulity ' + signalQulity + '"></div>'
		+ '<span style='+ textColor +'> ' + I18N("j", signalQulity) + '</span>'
		+ '</div>'
		+ '</div>'
		+ '</div>'
			// -- Settings Card
	...
	showClientInfo: function() {
		...
		*/
		...
		if(self.Type.toUpperCase().indexOf('GUEST') < 0) {
			outputString += '<div class ="client_EditImage" onclick ="editData(' + self.rowid + ')" style ="cursor:pointer"></div>';
		}
		

		outputString += '<div class ="client_Vender" title="'+vendor+'" style="overflow:hidden;text-overflow:ellipsis;width:100px;white-space:nowrap;">' + vendor + '</div>'
...
function showClientLists(type) {
	var column = 0;
	var clientItemsHTML = '<table class="block" border="0" ondragstart="return false" onselectstart="return false" width="900px"><tbody><tr>';
	for(var row in datalist.list) {
	...
	//add button
	column++;
	var addbutton = "<td>"+'<div class ="client_add_Tag" onclick ="addData()" style ="cursor:pointer;';
