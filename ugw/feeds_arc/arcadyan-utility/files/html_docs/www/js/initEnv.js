...
function getDeviceInfo() {
	...
	soapAction.sendSOAPAction("GetDeviceSettings", null, getDeviceSettings).done(function(obj){
		...
		sessionStorage.setItem('FWRegion', obj.FirmwareRegion);
		sessionStorage.setItem('RequireLevel', obj.RequireLevel);

		// Decide if to enable McAfee
		if(obj.SupportMyDLinkAbilities && obj.SupportMyDLinkAbilities.toLowerCase().indexOf("gen2") >= 0 && obj.SupportMyDLinkAbilities.toLowerCase().indexOf("mcafee") >= 0) {
			sessionStorage.setItem("McAfee", true);
		} else {
			sessionStorage.setItem("McAfee", false);
		}
...
function detectBrowserVersion() {
	...
	var version = parseInt($.client.version);
