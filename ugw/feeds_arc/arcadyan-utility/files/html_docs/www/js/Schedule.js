...
Datalist.prototype.checkData = function(newdata, rowNum){
	...
	for(i = 0; i < this.list.length; i++)
	{
		...
		{
			var nameArray = checkName.split("_");
			name = nameArray[0];
			...
			{
				var newDataNameArray = newdata.name.split("_");
...
RuleObj.prototype = 
{
	...
	setDataToRow : function(object)
	{
		...
		outputString += "<td><img src='image/edit_btn.png?v=2fc10c4616' width=28 height=28 style='cursor:pointer' onclick='editData("+this.rowid+")'/></td>";
		outputString += "<td><img src='image/trash.png?v=8fee37bb10' width=41 height=41 style='cursor:pointer' onclick='deleteData("+this.rowid+")'/></td>";
...
RuleObj.prototype.makeAllWeek = function()
{
	for(var i = 0; i < this.list.length; i++)
	{
		var tSet = this.list[i];
		var count = 0;
		if(tSet.date != 1)
			continue;
		
		for(var j = 0, max2 = this.list.length; j < max2; j++)
		{
			if((tSet.start == this.list[j].start)&&(tSet.end == this.list[j].end))
			{
				count++;
			}
		}
...
RuleObj.prototype.releaseAllWeek = function()
{
	for(var i = 0; i < this.list.length; i++)
	{
		if(this.list[i].date.toString() != "0")
			continue;

		for(var j = 2; j <= 7; j++)
		{
			var tSet = new TimeSet(j, this.list[i].start, this.list[i].end);
			this.list.push(tSet);
		}
		this.list[i].date = 1;
	}
