...
if(currentDevice.featureMyDLink) {
	menu 		+= "			<li><a href='javascript:CheckHTMLStatus(\"Mydlink\");' onclick='return confirmExit()'>"+I18N("j", "D-Link Cloud")+"</a></li>";	
}

if(currentDevice.featureOperationMode) {
	menu 		+= "			<li><a href='javascript:CheckHTMLStatus(\"OperationMode\");' onclick='return confirmExit()'>"+I18N("j", "Operation Mode")+"</a></li>";
}
...
if(currentDevice.featureQuickVPN) { 
	...
	if(currentDevice.TR069) {
		menu += "			<li><a href='javascript:CheckHTMLStatus(\"TR069\");' onclick='return confirmExit()'>" + I18N("j", "TR069") + "</a></li>";
	}
	if(currentDevice.featureNewTime){
		menu 		+= "			<li><a href='javascript:CheckHTMLStatus(\"Time2\");' onclick='return confirmExit()'>"+I18N("j", "Time & Schedule")+"</a></li>";
	} else {
...
if(currentDevice.featureUserDevice) {
	menu += "			<li><a href='javascript:CheckHTMLStatus(\"UserAndDevice\");' onclick='return confirmExit()'>" + I18N("j", "User") + "</a></li>";
}
if(currentDevice.featureMultiFirmware) {
	menu 		+= "			<li><a href='javascript:CheckHTMLStatus(\"UpdateFirmware_Multi\");' onclick='return confirmExit()'>"+I18N("j", "Firmware Upgrade")+"</a></li>";
} else {
	menu 		+= "			<li><a href='javascript:CheckHTMLStatus(\"UpdateFirmware\");' onclick='return confirmExit()'>"+I18N("j", "Firmware Upgrade")+"</a></li>";	
}
...
// dlink cloud
	...
	// 		if(getMyDLinkSettingsResponse.AccountStatus == 'true'){
	// 			$("#m_DlinkCloud").show();
	// 		}
	// 	});
	// }
...
{	
	if (localStorage.getItem('language') == "ru-ru")	{	document.getElementById(menuId).style.background = "url(./image/navigation_bg6.gif?v=08f2240644) right top no-repeat";	}
	else	{	document.getElementById(menuId).style.background = "url(./image/navigation_bg5.gif?v=e1ab0947f4) right top no-repeat";	}
}
