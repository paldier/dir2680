...
int32_t URLFilter_processDNSPkt(IN struct nfq_data * pxPkt, IN struct nfq_q_handle * pxQueue)
{
	...
	const struct url_sniffIP *pxIpHdr = NULL;	/* The IP header */
	...
	if (nLen) {
		...
		pxIpHdr = (struct url_sniffIP *)(pucPktData);
		ip6hdr = (struct ipv6hdr *)(pucPktData);
		version = ip6hdr->version;
		if (version == 4) {
			unIpHdrLen = IP_HDRLEN(pxIpHdr) * 4;
			if (unIpHdrLen < 20) {
